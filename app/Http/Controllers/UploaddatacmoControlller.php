...
use App\Models\UploaddatacmoModel;
use App\Models\User;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;
use Inertia\Inertia;

class UploaddatacmoControlller extends Controller
{
    // public function index(Request $request)
    // {
    //     // $reports = UploaddatacmoModel::with('user')->latest()->get();
    //     $reports = UploaddatacmoModel::with('user')
    //         ->latest()
    //         ->paginate(10);
    ...
    //     return Inertia::render('Uploaddatacmo/Index', [
    //         'reports' => $reports,
    //         'agents' => $agents,
    //     ]);
    // }
    ...
    public function index(Request $request)
    {
        $query = UploaddatacmoModel::with('user');
        ...
        $periods = UploaddatacmoModel::select('month', 'year')
            ->distinct()
            ->orderByDesc('year')
            ->orderByDesc('month')
            ->get();
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('file')) {
            ...
            $path = $file->store(
                "reports/datacmo/{$request->year}/{$request->month}",
                'public'
            );

            UploaddatacmoModel::create([
    ...
    public function update(Request $request, UploaddatacmoModel $report)
    {
        $rules = [
            'user_id' => 'required|exists:users,id',
            'month'   => 'required|integer|between:1,12',
    ...
    public function download(UploaddatacmoModel $report)
    {
    ...
    public function destroy(UploaddatacmoModel $report)
    {
