...
use App\Models\UploaddataagentModel;
use App\Models\User;
...
class UploaddataagentControlller extends Controller
{
    public function index(Request $request)
    {
        $query = UploaddataagentModel::with('user');
        ...
        $periods = UploaddataagentModel::select('month', 'year')
            ->distinct()
            ->orderByDesc('year')
            ->orderByDesc('month')
            ->get();

        return Inertia::render('Uploaddataagent/Index', [
            'reports' => $reports,
            'agents'  => $agents,
            'periods' => $periods,
            'filters' => [
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('file')) {
            ...
            $path = $file->store(
                "reports/dataagent/{$request->year}/{$request->month}",
                'public'
            );

            UploaddataagentModel::create([
    ...
    public function update(Request $request, UploaddataagentModel $report)
    {
        $rules = [
            'user_id' => 'required|exists:users,id',
            'month'   => 'required|integer|between:1,12',
    ...
    public function download(UploaddataagentModel $report)
    {
    ...
    public function destroy(UploaddataagentModel $report)
    {
