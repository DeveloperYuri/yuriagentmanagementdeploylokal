...
const submit = () => {
    if (isEditing.value) {
        ...
        router.post(
            route("datacmo.update", editingId.value),
            {
...
const downloadFile = (id) => {
    window.location.href = route("datacmo.download", id);
};
...
const executeDelete = () => {
    router.delete(route("datacmo.destroy", reportToDelete.value.id), {
        preserveScroll: true,
        onSuccess: () => {
...
const search = () => {
    ...
    router.get(route("datacmo.index"), params, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
    });
...
const resetFilter = () => {
    ...
    router.get(route("datacmo.index"));
};
</script>
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>Data Template Yuri Agent</template>
            ...
            <div class="grid grid-cols-12 gap-4 items-end">
                ...
                <div class="col-span-6">
                    <select
                        ...
                        class="w-full border-gray-300 rounded-md h-[42px]"
                    >
                ...
                <div class="col-span-1 flex">
                    <button
                        @click="resetFilter"
                        class="h-[42px] px-4 bg-slate-500 hover:bg-slate-600 text-white rounded-md text-sm whitespace-nowrap"
                    >
                        Reset
                    </button>
                </div>
                ...
                >
                    ...
                    <div class="flex items-center gap-1">
                        <button
                            ...
                            :disabled="!link.url"
                            v-html="link.label"
                            class="min-w-[38px] h-[40px] px-3 border rounded-md text-sm transition"
                            :class="[
                                link.active
